...
RDLogger.DisableLog('rdApp.*')  # ignore :: WARNING: not removing hydrogen atom without neighbors
...
def split_train_val_test_idx_stratified_v2(idx, y, frac_train=0.8, frac_valid=0.1, frac_test=0.1, sort=False, seed=42):
    import deepchem as dc
    dataset = dc.data.DiskDataset.from_numpy(X=idx, y=y)
    randomstratifiedsplitter = dc.splits.RandomStratifiedSplitter()
    train_idx, valid_idx, test_idx = randomstratifiedsplitter.split(dataset, frac_train, frac_valid, frac_test, seed=seed)
    print(f"train: {len(train_idx)}, valid: {len(valid_idx)}, test: {len(test_idx)}")
    return train_idx, valid_idx, test_idx


def scaffold_split_train_val_test(index, smiles_list, frac_train=0.8, frac_valid=0.1, frac_test=0.1, sort=False, include_chirality=True):
    np.testing.assert_almost_equal(frac_train + frac_valid + frac_test, 1.0)
...
def random_scaffold_split_train_val_test(index, smiles_list, frac_train=0.8, frac_valid=0.1, frac_test=0.1,
                                         sort=False, seed=42, include_chirality=True):
    np.testing.assert_almost_equal(frac_train + frac_valid + frac_test, 1.0)
...
def scaffold_to_smiles(mols: Union[List[str], List[Chem.Mol]],
                       ...
                       include_chirality=True) -> Dict[str, Union[Set[str], Set[int]]]:
    """
    ...
    for i, mol in tqdm(enumerate(mols), total=len(mols)):
        if Chem.MolFromSmiles(mol) != None:
            scaffold = generate_scaffold(mol, include_chirality=include_chirality)
            if use_indices:
...
def scaffold_split_balanced_train_val_test(index, smiles_list,
    ...
    scaffold_to_indices = scaffold_to_smiles(smiles_list, use_indices=True, include_chirality=include_chirality)
