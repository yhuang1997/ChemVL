...
def save_checkpoint(
    ...
    Args:
        ...
        model_name (str, optional): models name to save.
    Examples::
        >>> state = {
    ...
    epoch = state["epoch"]
    if not model_name:
        model_name = "models.pth.tar-" + str(epoch)
    ...
    if is_best:
        best_fpath = osp.join(osp.dirname(fpath), "models-best.pth.tar")
...
def resume_from_checkpoint(fdir, model, optimizer=None, scheduler=None):
    ...
    Args:
        fdir (str): directory where the models was saved.
    ...
    Examples::
        >>> fdir = 'log/my_model'
        >>> scheckpoint_filetart_epoch = resume_from_checkpoint(fdir, models, optimizer, scheduler)
    """
    start_epoch = 0
    ...
    if optimizer is not None and "optimizer" in checkpoint.mds():
        optimizer.load_state_dict(checkpoint["optimizer"])
        print("Loaded optimizer")

    if scheduler is not None and "scheduler" in checkpoint.mds():
...
def open_all_layers(model):
    r"""Open all layers in models for training.
    Examples::
...
def open_specified_layers(model, open_layers):
    r"""Open specified layers in models for training while keeping
    other layers frozen.
    Args:
    ...
    for layer in open_layers:
        assert hasattr(model, layer), '"{}" is not an attribute of the models, please provide the correct name'.format(
            layer
        )
...
def count_num_param(model):
    r"""Count number of parameters in a models.
    Args:
