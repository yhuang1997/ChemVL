#!/usr/bin/env bash
# MolMCL under ChemVL — MoleculeNet scaffold batch (GIN/GPS × cls/reg).
set -euo pipefail
...
if [[ "${SMOKE:-0}" == "1" ]]; then
  EXP_NAME="${EXP_NAME:-molmcl_moleculenet_smoke_scaffold}"
else
...
echo "=== MolMCL under ChemVL: scaffold exp=${EXP_NAME} RUN_GIN=${RUN_GIN} RUN_GPS=${RUN_GPS} PY=${PY} ==="

_fail=0
if [[ "$RUN_GIN" == "1" ]]; then
  if [[ "$RUN_CLS" == "1" ]]; then
    _run_batch configs/external/molmcl/molmcl_gin_moleculenet_classification.external.json "$DATASET_LIST_CLS" || _fail=1
  fi
  if [[ "$RUN_REG" == "1" ]]; then
    _run_batch configs/external/molmcl/molmcl_gin_moleculenet_regression.external.json "$DATASET_LIST_REG" || _fail=1
  fi
fi
if [[ "$RUN_GPS" == "1" ]]; then
  if [[ "$RUN_CLS" == "1" ]]; then
    _run_batch configs/external/molmcl/molmcl_gps_moleculenet_classification.external.json "$DATASET_LIST_CLS" || _fail=1
  fi
  if [[ "$RUN_REG" == "1" ]]; then
    _run_batch configs/external/molmcl/molmcl_gps_moleculenet_regression.external.json "$DATASET_LIST_REG" || _fail=1
  fi
fi
