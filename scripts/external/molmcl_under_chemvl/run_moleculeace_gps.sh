#!/usr/bin/env bash
# Batch MoleculeACE under ChemVL (``utils/external/finetune_external.py``).
#
# Examples:
...
#
# Optional: DRY_RUN=1 SAVE_GIT_DIFF=1 NO_SKIP=1
set -euo pipefail
...
BASE_CONFIG="${BASE_CONFIG:-configs/external/molmcl/molmcl_gps_moleculeace.external.json}"
...
echo "=== MolMCL under ChemVL: gps moleculeace exp=${EXP_NAME} PY=${PY} ==="
exec "${CMD[@]}" "$@"
