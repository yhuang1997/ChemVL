#!/usr/bin/env bash
# Plot finetuning trajectories for MolMCL-under-ChemVL batch runs (requires train_val_test_history.csv per run).
set -euo pipefail
...
"$PY" scripts/moleculeace_batch_plot_trajectories.py \
  --root "${RESULT_ROOT}" \
