#!/usr/bin/env bash
# ImageMol under ChemVL — scaffold split batch.
set -euo pipefail
...
BASE_CONFIG="${BASE_CONFIG:-configs/external/imagemol/imagemol_moleculenet_scaffold.external.json}"
...
echo "=== ImageMol under ChemVL: scaffold exp=${EXP_NAME} ==="
exec "${CMD[@]}" "$@"
