#!/usr/bin/env python3
"""
Batch driver for MolCLR / ImageMol under ChemVL (``utils/external/finetune_external.py``).

Merges per-task metadata from ``--task-dir`` (``configs/external/moleculenet/datasets``)
so classification vs regression dataroots and ``num_tasks`` resolve correctly.

Example::

    python scripts/external/molclr_under_chemvl/batch_run_external.py \\
      --base-config configs/external/molclr/molclr_gin_molclr_moleculenet_scaffold.external.json \\
      --dataset-list configs/external/moleculenet/dataset_list_moleculenet10.txt \\
      --task-dir configs/external/moleculenet/datasets \\
      --runseed-start 1 --runseed-end 3 \\
      --exp-name baseline-under-chemvl-scaffold
"""
...
GROUP_ID = "baseline_under_chemvl"
...
def load_task_overlay(task_dir: Path, task_key: str, *, backend_name: str = "") -> Dict[str, Any]:
    ...
    if raw.get("dataset"):
        ds = dict(raw["dataset"])
        # ImageMol ``get_datasets()`` expects ``.../MPP/{classification|regression}``.
        # Graph backends use parent dataroot + ``resolve_moleculenet_csv`` search.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--base-config", required=True, help="Base external JSON (merged first).")
    p.add_argument("--dataset-list", default="configs/external/moleculenet/dataset_list_moleculenet10.txt")
    p.add_argument("--task-dir", default="configs/external/moleculenet/datasets")
    p.add_argument("--datasets", default="", help="Comma-separated task keys (overrides --dataset-list).")
    ...
    p.add_argument("--finetune-script", default=None)
    p.add_argument("--no-skip-existing", action="store_true")
    p.add_argument("--exp-name", default="baseline-under-chemvl-scaffold")
    p.add_argument("--save-git-diff", action="store_true")
    return p.parse_args()


def resolve_datasets(args: argparse.Namespace, root: str) -> List[str]:
...
def main() -> int:
    ...
    backend_name = str((base_cfg.get("model") or {}).get("finetune_backend", ""))
    ...
    try:
        datasets = resolve_datasets(args, root)
    ...
    for task_key in datasets:
        for runseed in range(args.runseed_start, args.runseed_end + 1):
            try:
                task_overlay = load_task_overlay(task_dir, task_key, backend_name=backend_name)
            except FileNotFoundError as e:
                print(str(e), file=sys.stderr)
                failures += 1
                continue
            ...
            fd, tmp_overlay = tempfile.mkstemp(
                prefix=f"baseline_chemvl_overlay_{task_key}_{runseed}_", suffix=".json"
            )
            os.close(fd)
            try:
                ...
                if args.dry_run:
                    ...
                    print(
                        ...
                        f"backend={(cfg.get('model') or {}).get('finetune_backend')}"
                    )
                    continue

                fd2, tmp_merged = tempfile.mkstemp(
                    prefix=f"baseline_chemvl_merged_{task_key}_{runseed}_", suffix=".json"
                )
                os.close(fd2)
                try:
